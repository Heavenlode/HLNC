...
namespace HLNC
{
	/**
		...
		On every network tick, all NetNode3D nodes in the scene tree automatically have their <see cref="NetProperty">network properties</see> updated with the latest data from the server.
	...
	*/
	...
	public partial class NetNode3D : Node3D, INetNode, INotifyPropertyChanged, INetSerializable<NetNode3D>, IBsonSerializable<NetNode3D>
	{
		...
		public static NetNode3D BsonDeserialize(Variant context, byte[] bson, NetNode3D obj)
		{
			...
			node.Network._prepareNetId = NetId.BsonDeserialize(context, DataTransformer.Instance.SerializeBsonValue(doc["NetId"]), node.Network.NetId);
