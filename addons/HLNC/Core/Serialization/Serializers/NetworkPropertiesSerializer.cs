...
namespace HLNC.Serialization.Serializers
{
    ...
    internal partial class NetworkPropertiesSerializer : Node, IStateSerializer
    {
        private struct Data
        {
        ...
        public override void _EnterTree()
        {
            ...
            else
            {
                ...
                {
                    var prop = NetworkScenesRegister.UnpackProperty(wrapper.Node.SceneFilePath, propIndex);
        ...
        private Data Deserialize(HLBuffer buffer)
        {
            ...
            for (byte propertyByteIndex = 0; propertyByteIndex < data.propertiesUpdated.Length; propertyByteIndex++)
            {
                ...
                for (byte propertyBit = 0; propertyBit < BitConstants.BitsInByte; propertyBit++)
                {
                    ...
                    var prop = NetworkScenesRegister.UnpackProperty(wrapper.Node.SceneFilePath, propertyIndex);
                    // GD.Print($"FOR {wrapper.Node.GetPath()}: Receiving prop update: {prop.Name}");
                    if (prop.Type == Variant.Type.Object)
                    {
                        var node = wrapper.Node.GetNode(prop.NodePath);
                        data.properties[propertyIndex] = prop.NetworkDeserialize.Call(wrapper.CurrentWorld, buffer, node.Get(prop.Name));
                    }
                    else if (prop.Subtype == VariantSubtype.NetworkId)
                    {
                        data.properties[propertyIndex] = HLBytes.UnpackByte(buffer);
                    }
                    else
                    {
        ...
        {
            return (NetworkScenesRegister.GetPropertyCount(wrapper.Node.SceneFilePath) / BitConstants.BitsInByte) + 1;
        }
        ...
        public HLBuffer Export(WorldRunner currentWorld, NetPeer peerId)
        {
            ...
            for (var i = 0; i < propertiesUpdated.Length; i++) {
                ...
                for (var j = 0; j < BitConstants.BitsInByte; j++)
                {
                    ...
                    if (prop.Type == Variant.Type.Object)
                    {
                        HLBytes.Pack(buffer, prop.NetworkSerialize.Call(currentWorld, peerId, varVal).As<HLBuffer>().bytes);
                    }
                    else if (prop.Subtype == VariantSubtype.NetworkId)
                    {
                        varVal = currentWorld.GetPeerWorldState(peerId).Value.WorldToPeerNodeMap.GetValueOrDefault<long, byte>(varVal.AsInt64(), 0);
                        HLBytes.Pack(buffer, varVal.AsByte());
                    }
                    else
                    {
