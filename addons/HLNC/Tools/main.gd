...
const DockNetScenes = preload("res://addons/HLNC/Tools/Dock/NetScenes/dock_net_scenes.tscn")
...
const DataTransformer = preload("res://addons/HLNC/Utils/Bson/DataTransformer.cs")
const NetSceneInspector = preload("res://addons/HLNC/Tools/Inspector/NetSceneInspector.cs")
const AddonManager = preload("res://addons/HLNC/Tools/AddonManager/addon_manager.tscn")
const ToolMenu = preload("res://addons/HLNC/Tools/tool_menu.tscn")
...
func _build():
    ...
    if !build_result:
        printerr("Failed to build HLNC protocol")
        return false
...
func _enter_tree():
    ...
    add_tool_submenu_item("HLNC", tool_menu_instance)

    add_autoload_singleton(AUTOLOAD_DEBUGGER, "res://addons/HLNC/Utils/Debugger/Debugger.cs")
    add_autoload_singleton(AUTOLOAD_ENV, "res://addons/HLNC/Utils/Env/Env.cs")
    add_autoload_singleton(AUTOLOAD_DATA_TRANSFORMER, "res://addons/HLNC/Utils/Bson/DataTransformer.cs")
    add_autoload_singleton(AUTOLOAD_PROTOCOL_REGISTRY, "res://addons/HLNC/Core/Serialization/ProtocolRegistry.cs")
    add_autoload_singleton(AUTOLOAD_RUNNER, "res://addons/HLNC/Core/NetRunner.cs")
    add_autoload_singleton(AUTOLOAD_PROTOCOL_REGISTRY_BUILDER, "res://addons/HLNC/Core/Serialization/ProtocolRegistryBuilder.cs")
    ...
    # add_custom_type("NetTransform3D", "Node", preload("res://addons/HLNC/Core/Nodes/NetTransform/NetTransform3D.cs"), network_transform_3d_icon)
    # var network_transform_2d_icon = EditorInterface.get_editor_theme().get_icon("RemoteTransform2D", "EditorIcons")
    # add_custom_type("NetTransform2D", "Node", preload("res://addons/HLNC/Core/Nodes/NetTransform/NetTransform2D.cs"), network_transform_2d_icon)
