    ...
    public partial class BsonTransformer : Node
    {
        ...
        )
        {
            ...
            foreach (var node in ProtocolRegistry.Instance.ListProperties(network.Owner.Node.SceneFilePath))
            {
                ...
                var nodeProps = node["properties"].As<Godot.Collections.Array<CollectedNetProperty>>();
                ...
                foreach (var property in nodeProps)
                {
                    ...
                    var val = Serialization.BsonSerialize.SerializeVariant(context, prop, property.Metadata.TypeIdentifier);
